...
%% Просто скопировал из прошлой лабы
append([],L,L). 
...
print_tree(X) :- nl, print_tree(X, 0). 
print_tree(empty, _) :- write(empty), nl.
...
balanced_tree(L, T) :- quicksort(L, Sorted), build_tree(Sorted, T), print(T), print_tree(T).
...
:- balanced_tree([4221, -3, 15, -10, 20], T).

%% Результат:
%% :- balanced_tree([4221, -3, 15, -10, 20], T).
%% instant(15,instant(-3,instant(-10,empty,empty),empty),instant(4221,instant(20,empty,empty),empty))
%% 15
%%         -3
%%                 -10
%%                 empty
